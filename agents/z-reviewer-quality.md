...
- **Confidence**: 0-100% — how certain you are, after re-reading the cited code, that this is a real defect. Findings below 70% are not dropped — `/z-review` sends them to a separate verification step. Report uncertain findings with an honest number rather than leaving them out.
